...
bool shabalin::Triangle::isTriangle(point_t p1, point_t p2, point_t p3)
{
...
double shabalin::Triangle::distance(point_t p1, point_t p2)
{
  return std::sqrt(std::pow(p2.x - p1.x, 2.0) + std::pow(p2.y - p1.y, 2.0));
}

void shabalin::Triangle::move(point_t p)
{
...
void shabalin::Triangle::move(double shift_x, double shift_y)
{
  p1_.x += shift_x;
  p1_.y += shift_y;
  p2_.x += shift_x;
...
void shabalin::Triangle::scale(double scale)
{
  double dx = p1_.x;
  double dy = p1_.y;
...
rectange_t shabalin::Triangle::getFrameRect() const
{
